...
本系统采用 SQLite FTS5（Full-Text Search 5）全文索引技术实现高性能笔记搜索功能，相比传统的 LIKE 模糊查询，搜索性能提升 **最高 53,279 倍**，能够轻松应对百万级数据量的实时搜索需求。
...
|--------|--------------|--------------|----------|
| 1,000条 | 6.22ms | 0.22ms | **29倍** |
| 10,000条 | 69.19ms | 0.16ms | **431倍** |
| 100,000条 | 620.75ms | 0.14ms | **4,350倍** |
| 500,000条 | 3,264.58ms | 0.15ms | **22,393倍** |
| 1,000,000条 | 6,871.04ms | 0.13ms | **53,279倍** |

#### 详细测试数据（含最小/最大值）

| 数据量 | LIKE最小 | LIKE平均 | LIKE最大 | FTS5最小 | FTS5平均 | FTS5最大 |
|--------|----------|----------|----------|----------|----------|----------|
| 1,000条 | 5.51ms | 6.22ms | 6.75ms | 0.19ms | 0.22ms | 0.25ms |
| 10,000条 | 60.91ms | 69.19ms | 76.21ms | 0.11ms | 0.16ms | 0.25ms |
| 100,000条 | 617.69ms | 620.75ms | 627.28ms | 0.12ms | 0.14ms | 0.22ms |
| 500,000条 | 3,179.38ms | 3,264.58ms | 3,455.01ms | 0.12ms | 0.15ms | 0.19ms |
| 1,000,000条 | 6,717.98ms | 6,871.04ms | 7,045.32ms | 0.11ms | 0.13ms | 0.16ms |
...
1000│ ████████████████ (LIKE 100K)
 ...
 0.2│ ▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪ (FTS5 所有规模)
    └────────────────────────────────────────────────────────────
         1K     10K    100K    500K    1M    数据量
```

### 测试结论

- **FTS5 查询时间恒定**：无论数据量多少，查询时间稳定在 **0.13-0.22ms**
- **LIKE 查询线性增长**：数据量翻倍，耗时接近翻倍
- **百万级数据 FTS5 快 53,279 倍**：差距随数据量增大呈指数级扩大
- **结论**：数据量越大，FTS5 优势越明显
...
|------|--------------|--------------|------|
| 万级数据搜索 | 69.19ms | 0.16ms | **431倍** |
| 十万级数据搜索 | 620.75ms | 0.14ms | **4,350倍** |
| 百万级数据搜索 | 6,871.04ms | 0.13ms | **53,279倍** |
| 中文支持 | 部分 | 完美 | ✓ |
| 多关键词搜索 | 慢 | 快 | ✓ |
| 前缀匹配 | 不支持 | 支持 | ✓ |
| 智能降级 | - | 支持 | ✓ |
